...
std::vector<std::string> depth_first_search(const Problem &problem) {
    size_t max_memory = 0;
    ...
    if (problem.goal_test(root->state)) {
        std::cout << "Estado inicial é o objetivo!" << std::endl;
        return {};
    }
    ...
    while (!frontier.empty()) {
        ...
        // Expandir ações (ordem reversa para DFS manter ordem lógica)
        ...
        for (auto &action_pair : actions) {
            ...
            // Verificar se child_state está na fronteira
            bool in_frontier = false;
            std::stack<Node*> temp = frontier;
            while (!temp.empty()) {
                if (temp.top()->state.VerifyState(child_state)) {
                    in_frontier = true;
                    break;
                }
                temp.pop();
            ...
            if (explored.count(child_state) == 0 && !in_frontier) {
                ...
                if (problem.goal_test(child->state)) {
                    ...
                    std::cout << "Busca em Profundidade (DFS) concluída." << std::endl;
